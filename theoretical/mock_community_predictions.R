...
for (ii in 1:30) {
    ...
    mock[ii, 2:(mock[ii, "num_bugs"]+1)] <- (rpois(mock[ii,"num_bugs"], 
                                                   mock[ii,"parameter"]))
...
babies2 <- read.csv(
  "/Users/danielle/Documents/thesis/theoretical/theoretical_babies_df.csv")
...
mean.under15 <- mean(na.omit(
  babies2[babies2$dev_stage == "less than 15 months",]$read_predictions))
var.under15 <- var(na.omit(
  babies2[babies2$dev_stage == "less than 15 months",]$read_predictions))
...
var.over15 <- var(na.omit(
  babies2[babies2$dev_stage == "15 to 30 months" 
              |babies2$dev_stage == "older than 30 months",]$read_predictions))
...
# boxplots
babies3 <- subset(babies2, select=c("sampleid", 
                                    "dev_stage", "read_predictions"))
colnames(babies3)[3] <- "predicted necessary depth"
babies3 <- melt(babies3)

babies4 <- subset(babies2, select=c("sampleid", 
                                    "dev_stage", "read_depth"))
...
plot2 <- ggplot(data=subset(babies2, !is.na(dev_stage)), 
                aes(log(richness), evenness))
